...
contract("OptimizedRelayer", function ([operator, relayee]) {
  const relayeeWallet = web3.eth.accounts.privateKeyToAccount(PRI_KEY);
  let relayer;
  let hello;

  beforeEach(async function () {
    relayer = await OptimizedRelayer.new();
  ...
  describe("execute", () => {
    it("mesure gas cost 3 times", async function () {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        receipts.push(
          await relayer.execute(relayee, hello.address, abiEncodedCall, sig.v, sig.r, sig.s, {
            from: operator,
          })
        );
      }
